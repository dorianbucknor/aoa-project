...
class RedBlackTree {
    ...
    protected void insertNode(Book book) {
        ...
        while (node != null) {
            ...
            if (compareResult < 0) {
                ...
                throw new IllegalArgumentException("BST already contains a node with key " + book.indexer);
            }
        }
    ...
    private void recolor(Node node) {
        ...
        // Note on performance:
        // It would be faster to do the uncleNode color check within the following code. This way
        // we would avoid checking the grandparent-nodeParent direction twice (once in getUncle()
        // and once in the following else-if). But for better understanding of the code,
        // I left the uncleNode color check as a separate step.
